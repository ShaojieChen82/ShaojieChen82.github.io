/* Case-study tuning v3
   Generalizes the C7 Active Aero editorial layout across Motorsport and CHP sections,
   compacts archive rails, disables rail auto-advance, and cleans video captions/titles. */
...
(() => {
  ...
  function tuneSection(sectionId, config) {
    ...
    if (!section || section.dataset.caseTune === 'v3') return true;
    ...
    const bottomCards = cards.slice(
      1 + config.leftSupportCount,
      1 + config.leftSupportCount + config.bottomCount
    );
  ...
  function renameTrackVideos() {
    ...
    document.querySelectorAll('#track-videos .youtube-card').forEach((card, index) => {
      const label = labels[index];
      if (!label) return;
      const heading = card.querySelector('h3');
      const iframe = card.querySelector('iframe');
      if (heading && heading.textContent !== label) heading.textContent = label;
      if (iframe && iframe.title !== label) iframe.title = label;
    });
  }

  function halveRailValue(card, cssName, dataName) {
    const value = parseFloat(card.style.getPropertyValue(cssName));
  ...
  function halveRailCard(card) {
    halveRailValue(card, '--rail-item-height', 'railHalfHeightV3');
    halveRailValue(card, '--rail-item-height-mobile', 'railHalfMobileV3');
    halveRailValue(card, '--rail-item-width', 'railHalfWidthV3');
  }

  function patchRailCard(card) {
    if (!card) return;

    if (card.dataset.railTuneV3 !== '1') {
  ...
  function tuneAll(attempt = 0) {
    ...
    const railsReady = Boolean(document.querySelector('.media-archive .media-dock.showcase-rail-v7'));
    if ((!ready || !railsReady) && attempt < 120) {
