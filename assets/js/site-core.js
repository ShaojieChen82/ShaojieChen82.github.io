...
const MEDIA_GROUPS = {
  ...
  professionalE8kw: ["assets/img/professional/E8kW_WhiteUnit.jpg", "assets/img/professional/EPS_mCHP_poster.jpg"],
  professionalE200: ["assets/img/professional/E200.jpg", "assets/img/professional/E200Diagram.png"],
  professionalPower: ["assets/img/professional/testing inverter.JPG"],
  professionalControls: ["assets/img/professional/load emulator thermal side.JPG", "assets/img/professional/motorized load bank with custom made PCB.JPG"],
  professionalFleet: ["assets/img/professional/Websupervisor.png"],
  c7Aero: [
    "assets/img/motorsport/galleries/active-aero-install.JPG",
    "assets/img/motorsport/galleries/Me Standing on the Front Splitter 1.jpg",
    "assets/img/motorsport/galleries/showing the front splitter ramp High df vs low drag.JPG",
...
function mediaTitle(path) {
  const filename = decodeURIComponent(path.split("/").pop() || path); if (EXACT_MEDIA_TITLES.has(filename)) return EXACT_MEDIA_TITLES.get(filename);
  const bare = filename.replace(/\.[^.]+$/, ""); const iteration = bare.match(/(?:^|\s)(\d+)$/)?.[1] || ""; const lower = bare.toLowerCase(); const suffix = iteration ? ` — iteration ${iteration}` : "";
